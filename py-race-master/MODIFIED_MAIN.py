...
#load traffic
    ...
    lap_count = 0
    lap_started = False  # Prevents multiple counts per pass
    finish_line_rect = pygame.Rect(target.x, target.y, 100, 100)  # Finish line at target position
    finish_line_img = load_image("finish.png", True)
...
#Check for menu/reset, (keyup event - trigger ONCE)
        # Lap count logic - update finish line position to match target
        finish_line_rect.x = target.x
        finish_line_rect.y = target.y
        
        car_center_x = car.x + 25
        car_center_y = car.y + 25
        if finish_line_rect.collidepoint(car_center_x, car_center_y):
            if not lap_started:
                lap_count += 1
                lap_started = True
                if lap_count == 1:
...
#Blit Blit..       
        ...
        # Draw finish line image (offset by camera)
        try:
            screen.blit(finish_line_img, (finish_line_rect.x - cam.x, finish_line_rect.y - cam.y))
        except Exception as e:
            print(f"Error drawing finish line: {e}")

        # Render lap count (top-left, ensure it's visible)
        try:
            lap_text = font.render(f"Laps: {lap_count}", True, (255, 255, 0))
            screen.blit(lap_text, (20, 100))
        except Exception as e:
            print(f"Error rendering lap count: {e}")

        # Render car position (bottom-right)
        try:
            car_pos_text = font.render(f"Pos: ({int(car.x)}, {int(car.y)})", True, (255, 255, 255))
            screen_width = screen.get_width()
            screen_height = screen.get_height()
            text_rect = car_pos_text.get_rect(bottomright=(screen_width - 20, screen_height - 20))
            screen.blit(car_pos_text, text_rect)
        except Exception as e:
            print(f"Error rendering car position: {e}")
